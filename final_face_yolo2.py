...
TIMEOUT_THRESHOLD = 30  # Timeout threshold in seconds (for re-evaluation)
...
DATA_FILE = "known_faces2.pkl"
known_embeddings = []
known_ids = []
id_counter = 1
stable_person_map = {}  # Persistent mapping of persons to their assigned IDs
face_history = defaultdict(lambda: deque(maxlen=HISTORY_LENGTH))  # History of embeddings
last_seen_time = {}  # To store the last time the person was seen
...
while cap.isOpened():
    ...
    for face_box, embedding in face_embeddings.items():
        ...
        if best_match_person_box:
            # Track face over time and smooth ID assignment
            ...
            # Add embedding to history for temporal smoothing
            if tuple(face_box) not in face_history:
                face_history[tuple(face_box)] = deque(maxlen=5)  # Track for the last 5 frames
            face_history[tuple(face_box)].append(embedding)

            # Check if the face embedding is stable across frames
            if len(face_history[tuple(face_box)]) == 5:
                avg_embedding = np.mean(np.array(face_history[tuple(face_box)]), axis=0)
                similarities = cosine_similarity([avg_embedding], known_embeddings)
                max_sim = np.max(similarities)
                best_match_index = np.argmax(similarities)
                ...
                stable_person_map[tuple(face_box)] = recognized_id
